            ...
            reply_markup=InlineKeyboardMarkup(
                [[
                    InlineKeyboardButton(
                        "📥 DOWNLOADED(تحميل) {:.2f}% 📥".format(current / total * 100),
    ...
    try:
        ...
        await bot.edit_inline_reply_markup(
            ...
            reply_markup=InlineKeyboardMarkup(
                [[
                    InlineKeyboardButton(
                       "🍪 تحضير بيانات 🍪", callback_data=f"{callbackQuery.data}"
                    )
                ],[
                    InlineKeyboardButton(
                        "🗑️ الالغاء 🗑️", callback_data=f"c{callbackQuery.data[1:]}")
        ...
        await bot.edit_inline_reply_markup(
            ...
            reply_markup=InlineKeyboardMarkup(
                [[
                    InlineKeyboardButton(
                        "🐍 بدا رفع 🐍", callback_data=f"{callbackQuery.data}",)
                ],[
                    InlineKeyboardButton(
                        "🗑️ الالغاء 🗑️", callback_data=f"c{callbackQuery.data[1:]}")
                ]]
        ...
        await bot.edit_inline_media(
            ...
            reply_markup=InlineKeyboardMarkup(
                [[
                    InlineKeyboardButton(
                        text="♻️ ابحث مرة أخرى ♻️", switch_inline_query_current_chat="",)
                ]]
            ),
        )
